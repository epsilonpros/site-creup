...
export function Button({
  ...
  const variants = {
    ...
    secondary: 'bg-primary-400/20 backdrop-blur-sm text-white border border-primary-300/30 hover:bg-primary-400/30',
  ...
  return (
    <button className={styles} {...props}>
      ...
      {Icon && iconPosition === 'right' && (
        <Icon className="w-4 h-4 ml-2 flex-shrink-0 transition-transform group-hover:translate-x-1" />
      )}
    </button>
  );
}